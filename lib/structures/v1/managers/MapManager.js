...
class MapManager {
  ...
  async addMapToRotation(mapId, afterMap = "", afterMapOrdinal = "") {
    ...
    return this.client._send(`RotAdd ${mapId} ${afterMap} ${afterMapOrdinal}`);
  }
  ...
  async removeMapFromRotation(mapId, mapOrdinal) {
    ...
    return this.client._send(`RotDel ${mapId} ${mapOrdinal ? mapOrdinal : ""}`);
  }
  ...
  async toggleMapShuffle() {
    return this.client._send("ToggleMapShuffle");
  }
