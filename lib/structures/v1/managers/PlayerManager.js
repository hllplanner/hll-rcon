...
class PlayerManager {
  ...
  async get(playerName) {
    ...
    const [retrievedPlayerName, playerId, playerFaction, playerRole, playerUnitId, playerUnitName, playerLoadout, kills, deaths, combatScore, offensiveScore, defensiveScore, supportScore] = regexp.exec(response).slice(1, 14);
  ...
  async message(playerId, message) {
    ...
    return this.client._send(`Message ${intoQuotes(playerId)} ${message}`, { shortResponse: true });
  }
  ...
  async punish(playerName, reason) {
    ...
    return this.client._send(`Punish ${intoQuotes(playerName)} ${reason ? intoQuotes(reason) : ""}`, { shortResponse: true });
  }
  ...
  async kick(playerId, reason) {
    ...
    return this.client._send(`Kick ${intoQuotes(playerId)} ${reason ? intoQuotes(reason) : ""}`, { shortResponse: true });
  }
  ...
  async tempBan(playerId, duration = 2, reason, adminName) {
    ...
    return this.client._send(`TempBan ${intoQuotes(playerId)} ${duration} ${reason ? intoQuotes(reason) : ""} ${adminName ? intoQuotes(adminName) : ""}`, { shortResponse: true });
  }
  ...
  async removeTempBan(playerId) {
    ...
    return this.client._send(`PardonTempBan ${playerId}`, { shortResponse: true });
  }
  ...
  async listTempBans() {
    ...
    for (let tempBan of list.elements) {
      ...
      let [identifier, nickname, duration, year, month, day, hour, minute, second, reason, admin] = regexp.exec(tempBan).slice(1, 12);
      const bannedOn = new Date(year, month - 1, day, hour, minute, second);
  ...
  async permaBan(playerId, reason, adminName) {
    ...
    return this.client._send(`PermaBan ${intoQuotes(playerId)} ${intoQuotes(reason || "")} ${intoQuotes(adminName || "")}`, { shortResponse: true });
  }
  ...
  async removePermaBan(playerId) {
    ...
    return this.client._send(`PardonPermaBan ${playerId}`, { shortResponse: true });
  }
  ...
  async switchTeamOnDeath(playerName) {
    ...
    return this.client._send(`SwitchTeamOnDeath ${playerName}`, { shortResponse: true });
  }
  ...
  async switchTeamNow(playerName) {
    ...
    return this.client._send(`SwitchTeamNow ${playerName}`, { shortResponse: true });
  }
  ...
  async addVIP(playerId, nickname) {
    ...
    return this.client._send(`VipAdd ${intoQuotes(playerId)} ${intoQuotes(nickname || "")}`, { shortResponse: true });
  }
  ...
  async removeVIP(playerId) {
    ...
    return await this.client._send(`VipDel ${playerId}`, { shortResponse: true });
  }
