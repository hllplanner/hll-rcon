...
class LogManager {
   ...
   * @throws {Error} if backtrack is undefined or NaN.
   */
  async fetch(backTrack, filter = "") {
    if (!backTrack) throw new Error("Error fetching logs, did not receive an amount of seconds to backtrack.");
    if(typeof backTrack !== "number" || isNaN(backTrack)) throw new Error("Error fetching logs, backtrack must be an umber.")

    const response = await this.client._send(`ShowLog ${backTrack * 60} ${intoQuotes(filter || "")}`);

    if (response === "FAIL") throw new Error("Server error fetching logs.");
