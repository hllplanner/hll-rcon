...
class SessionManager {
  ...
  async setMap(mapId, ordinal) {
    ...
    const response = await this.client._send(`Map ${mapId} ${ordinal ? ordinal : ""}`);
   ...
   */
  async getMap() {
    const response = await this.client._send("Get Map");
  ...
  async setSectorLayout(strongPoints) {
    if (strongPoints.length !== 5) throw new Error(`Must have 5 strong points, got ${strongPoints.length}: ${strongPoints}`);
    const mappedStrongPoints = strongPoints.map(sp => intoQuotes(sp));

    const response = await this.client._send(`GameLayout ${mappedStrongPoints.join(" ")}`);
  ...
  async broadcastMessage(message) {
    ...
    const response = await this.client._send(`Broadcast ${message}`);
   ...
   *
   * @returns {Promise<Object>} - An object containing game state information:
   *                              - `alliedPlayers` {number} - Number of allied players.
   *                              - `axisPlayers` {number} - Number of axis players.
   *                              - `alliesScore` {number} - Score of allied players.
   *                              - `axisScore` {number} - Score of axis players.
   *                              - `remainingTime` {string} - Remaining time in the game.
   *                              - `map` {string} - Current map name.
   *                              - `nextMap` {string} - Name of the next map.
   */
  async getGameState() {
