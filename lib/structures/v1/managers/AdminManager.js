...
class AdminManager {
  ...
  async add(playerId, group, comment) {
    ...
    const response = await this.client._send(`AdminAdd ${intoQuotes("playerId")} ${intoQuotes("group")} ${intoQuotes(comment)}`);
  ...
  async remove(playerId) {
    ...
    const response = await this.client._send(`AdminDel ${playerId}`);
