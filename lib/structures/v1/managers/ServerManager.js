...
class ServerManager {
  ...
  async setTeamSwitchCooldown(cooldown) {
    ...
    return this.client._send(`SetTeamSwitchCooldown ${cooldown}`);
  }
  ...
  async setMaxQueuedPlayers(count) {
    ...
    return this.client._send(`SetMaxQueuedPlayers ${count}`);
  }
  ...
  async setIdleKickCooldown(cooldown) {
    ...
    return this.client._send(`SetKickIdleTime ${cooldown}`);
  }
  ...
  async setPingThreshold(time) {
    ...
    return this.client._send(`SetHighPing ${time}`);
  }
  ...
  async setWelcomeMessage(message) {
    return this.client._send(`Say ${message}`);
  }
  ...
  async setAutoBalanceEnabled(enable) {
    ...
    return this.client._send(`SetAutoBalanceEnabled ${enable ? "on" : "off"}`);
  }
  ...
  async setAutoBalanceThreshold(difference) {
    ...
    return this.client._send(`SetAutoBalanceThreshold ${difference}`);
  }
  ...
  async setVoteKicksEnabled(enable) {
    if (typeof enable !== "boolean") throw new Error("Error setting vote kicks enabled, parameter must be a boolean.");

    return this.client._send(`SetVoteKickEnabled ${enable ? "on" : "off"}`);
  }
   ...
   *
   * @param thresholds {Array<Array<number>>}
   * @returns {Promise<"SUCCESS" | "FAIL">}
   */
  async setVoteKickThresholds(thresholds) {
    ...
    ) throw new Error("Error setting vote kick thresholds, thresholds must be a 2d-array, each child array must contain 2 numbers.");
    ...
    return this.client._send(`SetVoteKickThreshold ${thresholds.flat()}`);
  }
  ...
  async resetVoteKickThresholds() {
    return this.client._send("ResetVoteKickThreshold");
  }
  ...
  async setRCONPassword(oldPassword, newPassword) {
    return this.client._send(`RconPassword ${oldPassword} ${newPassword}`);
  }
  ...
  async setVIPSlotCount(count) {
    ...
    return this.client._send(`SetNumVipSlots ${count}`);
  }
  ...
  async addProfanities(profanities) {
    ...
    return this.client._send(`BanProfanity ${profanities.join(",")}`);
  }
  ...
  async removeProfanities(profanities) {
    ...
    return this.client._send(`UnbanProfanity ${profanities.join(",")}`);
  }
}
