...
class MapManager {
  ...
  async addMapToRotation(mapId, index) {
    ...
    if (response.statusCode === 200)
      ...
      throw new Error(`Error adding map to rotation: ${JSON.stringify(response)}`);
  }
  ...
  async removeMapFromRotation(index) {
    ...
    if (response.statusCode === 200)
      ...
      throw new Error(`Error removing map from rotation: ${JSON.stringify(response)}`);
  }
  ...
  async getMapSequence() {
    ...
    if (response.statusCode === 200)
      return response.contentBody.mAPS;
    else
      throw new Error(`Error removing map from rotation: ${JSON.stringify(response)}`);
  }
  ...
  async addMapToSequence(mapId, index) {
    ...
    if (response.statusCode === 200)
      ...
      throw new Error(`Error adding map to sequence: ${JSON.stringify(response)}`);
  }
  ...
  async removeMapFromSequence(index) {
    ...
    if (response.statusCode === 200)
      ...
      throw new Error(`Error removing map from sequence: ${JSON.stringify(response)}`);
  }
  ...
  async setMapShuffling(enable) {
    ...
    if (response.statusCode === 200)
      ...
      throw new Error(`Error setting map shuffle: ${JSON.stringify(response)}`);
  }
}
