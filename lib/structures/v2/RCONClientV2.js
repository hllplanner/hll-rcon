...
class RCONClientV2 extends EventEmitter {
  ...
  async _processQueue() {
    ...
    headerBuffer.writeUInt32LE(6456, 0);

    const messageBuffer = Buffer.concat([headerBuffer, wrappedBuffer]);

    let data;
    if (encrypt)
      data = Buffer.concat([headerBuffer, xor(wrappedBuffer, this.cipher)]);
    else
      data = messageBuffer;

    if (this.debug) console.log(`DEBUG[V2] - Attempting to write message: ${message.wrap()}`);
    this.socket.write(data);
    if (this.debug) console.log(`DEBUG[V2] - Wrote message: ${message.wrap()}`);

    this._timeout = setTimeout(() => {
      if (this._pendingReject) {
        this._pendingReject(new Error(`Timeout waiting on response for message: ${message.wrap()}`));
