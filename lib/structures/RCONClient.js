...
class RCONClient extends EventEmitter {
  ...
  /**
   * Constructs a new RCONClient instance.
   *
  ...
  async _processQueue() {
    ...
    headerBuffer.writeUInt32LE(0xDE450508, 0); // "Magic Number"
    headerBuffer.writeUInt32LE(id, 4);
    headerBuffer.writeUInt32LE(wrapped.length, 8);
  ...
  async _processBuffer() {
    ...
    const header = this.messageBuffer.subarray(0, 12);

    const messageId = header.readUInt32LE(4);
    const messageLength = header.readUInt32LE(8);

    if(this.messageBuffer.length < 12 + messageLength) return;

    let messageContentsBuffer = this.messageBuffer.subarray(12, messageLength + 12);
    if (this.cipher) {
      messageContentsBuffer = xor(messageContentsBuffer, this.cipher);
    }

    this.messageBuffer = this.messageBuffer.subarray(messageLength + 12);
