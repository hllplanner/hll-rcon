 ...
 * @property {RCONClientV1|null} v1 - The instance of the V1 client.
 * @property {RCONClientV2|null} v2 - The instance of the V2 client.
...
class IRCONClient extends EventEmitter {
  ...
  constructor({
    ...
    this.v2 = new RCONClientV2({ host, port, password, debug: debug === true || debug === 2 });

    this.v1.once("ready", () => {
      if (this.pollSource === 1 && this.enableLogPolling) {
        this.v1._initializeLogPolling({ pollWindow, pollFrequency });

        this.v1.on("newLog", (log) => {
    ...
    this.v2.once("ready", () => {
      if (this.pollSource === 2 && this.enableLogPolling) {
        this.v2._initializeLogPolling({ pollWindow, pollFrequency });

        this.v2.on("newLog", (log) => {
          this.emit("newLog", log);
  ...
  sendV1(message, options) {
    return this.v1._send(message, options);
  }
